...
//更新qq号
if (!empty($qq)){
    $DB->update('site',  ['qq' => $qq],['zid' => $userrow['zid']]);
}

//如果取消绑定就下次登录再提醒绑定
if ($bind != 2){
    //判断之前授权登录的用户 是否绑定了qq号 没绑定的提示绑定
    $userinfo = $DB->select('site', '*', ['zid' => $userrow['zid']]);
    if (!empty($userinfo[0]['qq_openid']) && is_numeric($userinfo[0]['qq']) == false){
        exit("<script type='application/javascript'>var tall = prompt('请输入您授权登录的qq号','');if (!tall){location.href = '/user?bind=2'}else{location.href ='/user?qq=' + tall}</script>");
    }
}
...
<div class="wrapper">
    <div class="modal fade" style="text-align: left;" id="userjs" tabindex="-1" role="dialog"
            ...
            <table class="table">
                ...
                <tr>
                    <th class="text-center">
                        <span style="color: #a9a9a9;">用户名</span><br><span
                                style="font-size: 18px;"><?php echo $user_name; ?></span>
                    </th>
                    <th class="text-center">
                        <span style="color: #a9a9a9;">UID</span><br><span
                                style="font-size: 18px;"><?php echo $userrow['zid'] ?></span>
                    </th>
                    <th class="text-center">
                        <span style="color: #a9a9a9;">今日收益</span><br><span style="font-size: 18px;"
                                                                           id="income_today">0元</span>
                    </th>
                </tr>
                <tr>
            ...
            <ul class="list-group no-radius">
                <?php if ($userrow['power'] > 0) { ?>
                    ...
                    <li style="font-weight:bold" class="list-group-item">
                        我的域名：
                        <a href="http://<?php echo $userrow['domain'] ?>/" target="_blank" rel="noreferrer">
                            <?php echo $userrow['domain'] ?>
                        </a><a href="uset.php?mod=site" class="btn btn-info btn-xs pull-right">编辑信息</a>
                    </li>
                    <?php if ($conf['fanghong_api']) { ?>
                        <li style="font-weight:bold" class="list-group-item">
                            防红链接：
                            <a href="javascript:;" id="copy-btn" data-clipboard-text="">
                                Loading...
                            </a>&nbsp;&nbsp;&nbsp;
                    ...
                    <li style="font-weight:bold" class="list-group-item">
                        网站名称：
                        <font color="blue">
                            <?php echo $userrow['sitename'] ?>
                        </font>
                    </li>
                    <li style="font-weight:bold" class="list-group-item">
                        站点类型：
                        <?php echo($userrow['power'] == 2 ? '<font color=red>专业版</font>' : '<font color=red>普及版</font>') ?>
                        &nbsp;
                        <?php
                ...
                <?php } else { ?>
                    <li style="font-weight:bold" class="list-group-item">
                        你还未开通分站
                        <br/>
                        <a href="regsite.php" class="btn btn-primary btn-sm btn-block">点此开通分站</a>
                    </li>
                <?php } ?>
            </ul>
        </div>
    ...
    function dopay(type) {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (res['code'] === 0) {
                    window.location.href = '../other/submit.php?type=' + type + '&orderid=' + res['trade_no'];
                } else {
                    layer.alert(res['msg']);
                }
            },
