...
if($mod == 'index'){
	if(isset($_GET['cid']) || isset($_GET['tid'])){
		$where = array();
		if(isset($_GET['cid'])) $where['cid'] = intval($_GET['cid']);
		if(isset($_GET['tid'])) $where['tid'] = intval($_GET['tid']);
		if(count($where) > 0){
			$seo_info = $DB->select('cate',['seo_title','seo_keywords','seo_description'],$where);
		}
	}
}
// =========== 添加代码结束
include Template::load($mod);

echo '<script>var _hmt = _hmt || [];(function() {var hm = document.createElement("script");hm.src = "https://hm.baidu.com/hm.js?2d70358a902c57eec3d87e96cf015b47";var s = document.getElementsByTagName("script")[0]; s.parentNode.insertBefore(hm, s);})();</script></div>';


//百度统计代码

call_action('xy_event_homeLoaded');
