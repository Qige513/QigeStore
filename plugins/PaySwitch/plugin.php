...
class PaySwitchPlugin extends PluginInterface
{
    ...
    public function beforeOrderSubmit($param)
    {
        ...
        if (1 != $param['only_balance']) {
            ...
            $result = [];
            if ($conf['alipay_api']) { // 全局开启才触发
                $result['pay_alipay'] = isset($data['data']['ali_pay']) ? $data['data']['ali_pay'] : $conf['alipay_api'];
            }
            if ($conf['wxpay_api']) { // 全局开启才触发
                $result['pay_wxpay']  = isset($data['data']['wx_pay']) ? $data['data']['wx_pay'] : $conf['wxpay_api'];
            }
            if ($conf['qqpay_api']) { // 全局开启才触发
                $result['pay_qqpay']  = isset($data['data']['qq_pay']) ? $data['data']['qq_pay'] : $conf['qqpay_api'];
            }
            return $result;
        }
        return false;
