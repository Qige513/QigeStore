...
//        }
        ...
        if ($zid->rowCount()) {
            ...
            $userToken = hash('sha256', md5($user . $pwd. SYS_KEY));
            //进行不可逆加密用户密码
    ...
    case 'paysite':
        ...
        if ($need == 0) {
            ...
            if ($zid->rowCount()) {
                $zid                = $islogin2 ? $userrow['zid'] : $DB->id();
                $_SESSION['newzid'] = $zid;
                unset($_SESSION['addsalt']);
                if (!$islogin2) {
                ...
                exit(json(['code' => 1, 'msg' => '开通分站成功', 'zid' => $zid]));
            } else {
                $err = $DB->error();
                if ($err == '[]') {
                    exitJson('开通分站失败');
                }
                exitJson('开通分站失败，错误信息：' . $err);
            }
        } else {
            if ($islogin2 == 1) {
            ...
            if ($flag->rowCount()) {
                ...
                $err = $DB->error();
                if ($err == '[]') {
                    exitJson('提交订单失败，亲联系相关人员');
                }
                exitJson('提交订单失败，错误信息：' . $err);
            }
        }
        break;
    case 'up_price':
        ...
        foreach ($list as $row) {
            ...
            $priceModel->setToolInfo($row['tid'], $row);
            //设置读取商品信息
            $price                      = $priceModel->getToolPrice($tid);
            $data[$row['tid']]['price'] = round($price * ($rate + 1), 2);

            if ($userrow['power'] == 2) {
                $costPrice                 = $priceModel->getToolCost($tid);
                $data[$row['tid']]['cost'] = round($costPrice * ($rate + 1), 2);
            }
            //如果为专业版则进行提升普及版价格
        }
...
//        log_result('前台用户签到', $reward, round($reward, 2), 1);
    ...
    case 'msglist':
        ...
        if ($page <= 0)
            $page = 1;
        $page--;
        $totalMessageCount = $DB->count('message', ['AND' => ['type' => $type, 'active' => 1]]);
        $msgRead           = explode(',', $userrow['msgread']);
        $limit             = 10;
        $ret               = $DB->select('message', '*', [
            'AND'   => [
                'type'   => $type,
                'active' => 1,
            ],
...
//        exitJson(var_dump($DB->last()));
    ...
    case 'recharge':
        ...
        if ($type !== 'alipay' && $type !== 'qqpay' && $type !== 'wxpay') {
            exitJson('支付类型不合法');
        }
        $value    = addslashes($_GET['value']);
        if (!is_numeric($value) || !preg_match('/^[0-9.]+$/', $value)) exitJson('提交参数错误');
        if ($type == 'wxpay' && $conf['recharge_wx_min'] > 0 && $value < $conf['recharge_wx_min']) {
            exitJson('最低充值金额为：￥' . $conf['recharge_wx_min']);
        }
        if ($type == 'alipay' && $conf['recharge_ali_min'] > 0 && $value < $conf['recharge_ali_min']) {
            exitJson('最低充值金额为：￥' . $conf['recharge_ali_min']);
        }
        if ($type == 'qqpay' && $conf['recharge_qq_min'] > 0 && $value < $conf['recharge_qq_min']) {
            exitJson('最低充值金额为：￥' . $conf['recharge_qq_min']);
        }
        // 钩子处理
        $hook_result = only_hook('beforeOrderSubmit', 'PayTools', ['only_balance' => 0, 'order_data' => $row]);
        if ($hook_result && isset($hook_result['pay_tools'])) {
            $hook_result = $hook_result['pay_tools'];
            if (isset($hook_result[$type]) && $hook_result[$type]['btn_type'] == 2) {
                exitJson(empty($hook_result['desc']) ? '微信支付通道暂时维护' : $hook_result['desc']);
            }
        }
        $trade_no = date('YmdHis') . rand(111, 999);
        $flag = $DB->insert('pay', [
    ...
    case 'call_plugin_ajax':
        ...
        break;
    case 'get_auth_bind_url':
        if (!$islogin2) {
            exitJson('未登录');
        }
        if (empty($userrow['qq_openid'])) { // 未绑定过
            $_SESSION['qq_bind'] = [$userrow['zid'] => false]; // 绑定
        } else {
            $_SESSION['qq_bind'] = [$userrow['zid'] => true]; // 解绑
        }
