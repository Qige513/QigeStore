...
switch ($act) {
    ...
    case 'pay':
        ...
        if ($tool && $tool['active'] == 1) {
            ...
            if ($method == 'cart_add') {
                ...
                // 数据返回覆盖
                foreach ($hook_result as $k => $v) {
                    if (is_array($v) && isset($v['order_data']))
                        $orderData = $v['order_data'];
                }
                ...
                $hook_result = hook('beforeOrderSubmit', ['only_balance' => $conf['forcermb'], 'order_data' => $orderData]);
                // 数据返回覆盖
                foreach ($hook_result as $plugin => $v) {
                    if ($plugin == 'PayToolsPlugin') { // 版本 v8.8.1 开始支持
                        $pay_tools = $v;
                    } elseif (is_array($v) && isset($v['order_data'])) {
                        $orderData = $v['order_data'];
                    } elseif ($plugin == 'PaySwitchPlugin') {
                        $pay_config = $v;
                    }
                }
                $flag = $DB->insert('pay', $orderData);
                if ($flag->rowCount()) {
                    ...
                    if ($conf['forcermb'] == 1) {
                        ...
                        $result['pay_wxpay'] = 0;
                        $result['pay_qqpay']  = 0;
                    } elseif (isset($pay_config)) { // 插件：支付切换插件
                        if (isset($pay_config['pay_alipay'])) {
                            $result['pay_alipay'] = $pay_config['pay_alipay'];
                        }
                        if (isset($pay_config['pay_wxpay'])) {
                            $result['pay_wxpay'] = $pay_config['pay_wxpay'];
                        }
                        if (isset($pay_config['pay_qqpay'])) {
                            $result['pay_qqpay'] = $pay_config['pay_qqpay'];
                        }

                    }

                    //若分类支付开关有开启
                    $config_info = $DB->get('config','*',['k' => 'pay_switch']);

                    if (!empty($config_info) && $config_info['v'] == 1){

                        //设置禁用支付方式
                        $class_info = $DB->get('class','*',['cid' => $tool['cid']]);
                        $result['pay_alipay'] = $result['pay_wxpay'] = $result['pay_qqpay'] = $result['pay_rmb'] = 0;

                        if (isset($class_info['pay_alipay']) && $class_info['pay_alipay'] == 1){
                            $result['pay_alipay'] = 1;
                        }
                        if (isset($class_info['pay_wxpay']) && $class_info['pay_wxpay'] == 1){
                            $result['pay_wxpay'] = 1;
                        }
                        if (isset($class_info['pay_qqpay']) && $class_info['pay_qqpay'] == 1){
                            $result['pay_qqpay'] = 1;
                        }
                        if($islogin2 == 1){
                            if (isset($class_info['pay_rmb']) && $class_info['pay_rmb'] == 1){
                                $result['pay_rmb'] = 1;
                            }
                        }
                    ...
                    // 插件：支付工具箱
                    if (isset($pay_tools) && is_array($pay_tools)) {
                        $result = array_merge($result, $pay_tools);
                    }
                    exit(json($result));
                } else {
    ...
    case 'card':
        ...
        if (!$myrow) {
            ...
            if ($tool && $tool['active'] == 1) {
                ...
                $srow['num']      = 1;
                $srow['zid']      = empty($siterow['zid']) ? 0 : $siterow['zid'];
                $srow['userid']   = $cookiesid;
    ...
    case 'query':
        ...
        foreach ($rs as $res) {
            $count++;
            $data[] = ['id' => $res['id'],'order_id' => $res['order_id'],'order_set' => $conf['order_set'], 'tid' => $res['tid'], 'input' => $res['input'],
    ...
    case 'order': //订单进度查询
        ...
        if ($tool['is_curl'] == 4 || $row['djzt'] == 3) { // 自动发卡密（第一个输入框为邮件格式才能发送）
            //自动发卡模式 或  djzt 对接状态
        ...
        } elseif ($tool['is_curl'] == 2) { // 自动提交到社区/卡盟
            ...
            if ($shequ['type'] == 1) {
                ...
                $list = sky_chadan($shequ['url'], $row['djorder']);
            }elseif ($shequ['type'] == 14) {
                $customerid = $shequ['username'];
                $key = $shequ['password'];
                $url = $shequ['url'];
                $orderid = $row['djorder'];
                $list = szw_chadan($url, $orderid,$customerid,$key);
            ...
            if (is_string($list) || is_null($list)) {
                $list          = [];
                $row['status'] = 5; // 已提交
            } else {
                ...
                if ($list['order_state'] == '未开始' || $list['order_state'] == '未知状态') {
                    $row['status'] = 5; // 已提交
                }
                if ($list['add_time'] == 0) {
                    $row['status'] = 5; // 已提交
        ...
        $result = [
            ...
            'money'    => $row['money'],
            'date'     => $row['addtime'],
            'inputs'   => showInputs($row, $input, $inputs),
            'list'     => $list,
            'kminfo'   => $kmdata,
            'alert'    => $tool['alert'],
            'desc'     => $tool['desc'],
    ...
    case 'getCommentKS':
        ...
        $result = getKuaiShouCommentListV2($id, $page);
        if ($result[0] == false)
            exit(json(['code' => -1, 'msg' => $result[1]]));
        exit($result[1]);
        break;
    case 'getshuoshuo':
            ...
            if ($verify_result && ($conf['alipay_api'] == 2 || $conf['qqpay_api'] == 2 || $conf['wxpay_api'] == 2 || $conf['tenpay_api'] == 2)) {
                ...
                if ($verifyData['trade_status'] == 'TRADE_FINISHED' || $verifyData['trade_status'] == 'TRADE_SUCCESS') {
                    ...
                    if ($srow['status'] == 1) {
                        ...
                        //需要判断此订单是否是自助开通分站订单
                        $selectResult = $DB->select('pay','*',['trade_no' => $out_trade_no]);
                        $selectZid = $selectResult[0]['zid'];
                        if ($selectResult[0]['name'] == '自助开通分站'){

                            $fenzhan_price2 = $DB->select('config','v',['k'=>'fenzhan_price2']);
                            $fenzhan_price = $DB->select('config','v',['k'=>'fenzhan_price']);

                            $selectInput = explode('|',$selectResult[0]['input']);
                            $point = round($money - $fenzhan_price2[0],2);
                            if ($selectInput[2] == 1){
                                $point = round($money - $fenzhan_price[0],2);
                            }
                            ...
                            $point_insert = $DB->insert('points', $insert_points_info);

                            //更新余额
                            $site_update = $DB->update('site', ['rmb[+]' => $point], ['zid' => $selectZid]);
                            $DB->pdo->commit();
                        }

                        if(!$point_insert || !$site_update){
                            $DB->pdo->rollBack();
                        }
                    ...
                    if ($srow['status'] == 0 && $srow['money'] == $money) {
                        ...
                        if ($updateResult->rowCount() > 0) {
                            ...
                            if ($selectResult[0]['name'] == '自助开通分站'){
                                ...
                                $DB->insert('points', $insert_points_info);

                                //更新余额
                                $DB->update('site', ['rmb[+]' => $point], ['zid' => $selectZid]);
                            }
    ...
    case 'invite_query':
        ...
        $input      = trim($input);
        $invite_row = $DB->get('invite', ['id', 'tid', 'date'], ['input[~]' => $input, 'ORDER' => ['id' => 'DESC']]);
        if (empty($invite_row)) {
            exitJson('暂无相关数据', -1);
        }
        $invite_row['name'] = $DB->get('tools', 'name', ['tid' => $invite_row['tid']]);
        if (empty($invite_row['name'])) {
            exitJson('推广商品已下架', -1);
        }
        $invite_row['rules'] = $DB->get('invite_rules', ['type', 'rule_value'], ['tid' => $invite_row['tid'], 'status' => 1]);
        if (empty($invite_row['rules'])) {
            exitJson('商品已关闭推广', -1);
        }
        $invite_row['count'] = $DB->count('invitelog', ['nid' => $invite_row['id'], 'status' => ($invite_row['rules']['type'] == 1 ? 2 : 1)]);
        $invite_row['info']  = $DB->get('invitelog', ['status'], ['nid' => $invite_row['id'], 'status' => 1]);
        if (empty($invite_row['info'])) {
            $invite_row['info'] = '';
        }
        exit(json(['code' => 0, 'msg' => 'success', 'data' => $invite_row]));
        break;
    case 'get_auth_url':
        unset($_SESSION['reload_login_gateway']);
        $model = new \ds\QQAuthConnect();
        $result = $model->getAuthOpenHostUrl();
        exit(json($result));
        break;
    case 'post_bind_old_user':
        if (!$islogin2) {
            exitJson('未登录');
        }
        $model = new \ds\QQAuthConnect();
        exit(json($model->bindOldSiteUser()));
        break;
    default:
        ...
        if (!$islogin2)
            $callEvent .= 'nopriv_';
        $callEvent .= $act;
        $ret       = call_action($callEvent);
        if ($ret['ret'] == 0) {
            echo json(['code' => -4, 'msg' => $ret['msg']]);
        } else if ($ret['ret'] == -1) {
            echo json(['code' => -5, 'msg' => $ret['msg']]);
        }
        break;
}
