        ...
        if ($mod == 'user_n') {
                    ...
                    <form action="./uset.php?mod=user_n" method="post" role="form">
                        <div class="form-group">
                            ...
                            <?php if (empty($userrow['qq_openid'])): ?>
                                ...
                                <a style="text-decoration: underline;" href="javascript:" id="qq_bind">绑定</a>
                            <?php else:?>
                                <?php echo $userrow['qq_nickname']; ?>&nbsp;&nbsp;<span class="label label-success">已绑定</span>&nbsp;
                                <a style="text-decoration: underline;" href="javascript:" id="qq_bind">解绑</a>
                            <?php endif; ?>
                        </div>
            ...
            <script>
                ...
                function getQQAuthUrl() {
                    layer.load(2);
                    $.ajax({
                        url: 'ajax.php?act=get_auth_bind_url',
                        success(res) {
                ...
                // QQ 绑定
                $('#qq_bind').click(function () {
                    const is_bind = '<?php echo !empty($userrow['qq_openid']); ?>';
                    if (is_bind) { // 已经绑定
                        layer.confirm('您确定取消绑定吗？解绑后还可以重新绑定哦', {icon: 3}, function () {
                            getQQAuthUrl();
                        });
                    } else {
                        getQQAuthUrl();
