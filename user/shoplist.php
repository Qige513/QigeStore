...
<div class="wrapper">
    <div class="col-sm-12">
        ...
        ?>
        <div class="modal fade" align="left" id="search2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
             aria-hidden="true">
    ...
    for (i = 0; i < items.length; i++) {
        $(items[i]).val($(items[i]).attr("default")||0);
            ...
            $affectRow = $DB->update('site', ['price' => null], ['zid' => $userrow['zid']])->rowCount();
            if ($affectRow > 0) {
                showmsg('重置成功！<br/><br/><a href="./shoplist.php">>>返回商品列表</a>', 1);
            } else {
                $errorMsg = $DB->error();
                if ($errorMsg == '[]')
                    showmsg('并无做出任何修改<br/><br/><a href="./shoplist.php">>>返回商品列表</a>', 1);
                else
        ...
        <div class="table-responsive">
            <table class="table table-striped b-t b-light">
                ...
                <tr>
                    <th>操作</th>
                    <th>商品名称</th>
                    <th>成本价格</th><?php if ($userrow['power'] == 2) { ?>
                        <th style="font-size:14px">下级价格</th><?php } ?>
                    <th>销售价格</th>
                    <th>状态</th>
                </tr>
                </thead>
