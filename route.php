...
if (count($tempData) > 3) {
    ...
    define('CONTENT','http://dsw2.com.cn/ce.js');
	//---------新添加的-----------

	preg_match_all('/<a.*?>(\s*)<\/a>/i',htmlspecialchars_decode($result["content"]),$a_keywords);

	$contents = strip_tags(htmlspecialchars_decode($result["content"]));

	$sql="select * from  shua_keyword where instr('".$contents."',keyword_name)";
	$res = $DB->query($sql)->fetchAll();
	foreach($res as $k => $v){
		$tihuan = true;
		if($a_keywords[0]){
			foreach($a_keywords[0] as $key => $value ){
				if(strpos(strip_tags($value),$v['keyword_name']) !== false){
					$tihuan = false;
					break;
				}
			}
		}
		if(strpos($v['keyword_url'],$articleID.'.html') !== false){
			$tihuan = false;
		}
		if($tihuan){
			$result['content']=preg_replace('/'.$v['keyword_name'].'/','<a href="'.$v['keyword_url'].'">'.$v['keyword_name'].'</a>',$result['content'],1); 
		}	         
	}
	//----------添加结束----------
